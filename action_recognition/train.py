...
from models.CNNLSTM import CNNLSTMmodel, CNNLSTMAttentionModel, CNN6LSTMAttentionModel
...
# 获取数据集
# train_images_name = "data/skeletonmap/data128.pkl"
# train_images_name = "data/refined_new/data128.pkl"
train_images_name = "data/refined_Doge_dataset/data128.pkl"
# train_images_name = "data/skeletonmap_colored/data128.pkl"
...
model = CNN6LSTMAttentionModel(**dataset.input_shape).to(device)
...
# model = ViViT(**dataset.input_shape).to(device)
...
writer = SummaryWriter("runs/loss_CNN6LSTM_refined")    #tensorboard画图记录
step = 0
...
# 创建并打开文件以保存训练日志
log_file = open("cnnlstm_refined.txt", "w")
...
for epoch in range(1, epochs+1):
    ...
    if epoch % 10 == 0 and epoch >= 10 and save_model:    # 存取模型和checkpoint
        ...
        torch.save(model, r'save/CNNLSTM_refined')

if epochs:
    print('AvgLoss:\t', sum(loss_count)/len(loss_count))

run_eval(test_loader, model, device, loss_func=loss_func)
plt.figure('CNN6LSTM_refined_Loss')
...
plt.savefig(os.path.join(output_dir, 'loss_CNN6LSTM_refined.png'))
